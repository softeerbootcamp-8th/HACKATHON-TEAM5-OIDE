...
- `ExtractionPostProcessor`는 모델 출력에 규칙을 적용합니다.
  ...
  - 통화를 확정할 수 없으면 방 기본 통화로 보정하고 검토 표시를 남깁니다.
  - 여러 스크린샷에 겹친 거래는 삭제하지 않고 두 번째 항목부터 선택만 해제합니다.
...
- N빵은 `EqualShareCalculator`가 내림으로 기본 금액을 만든 뒤 남은 금액을 결제자에게 배정합니다. 결제자가 그룹에 없으면 표시 순서가 가장 빠른 참여자가 받습니다.
- 직접 입력은 그룹 구성원이 모두 포함되고, 음수가 없으며, 부담액 합계가 원 결제 금액과 정확히 같을 때만 저장합니다.
- 그룹 구성원이 바뀌면 EQUAL은 새 인원으로 다시 계산합니다. CUSTOM은 기존 금액을 최대한 유지하되 합계가 깨지면 EQUAL로 전환합니다.
